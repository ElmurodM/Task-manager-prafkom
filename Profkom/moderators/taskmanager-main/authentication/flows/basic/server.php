<?php 
	...
	if (isset($_POST['reg_user'])) {
      ...
      if($result){
		...
		if (count($errors) == 0) {
			$query = "INSERT INTO moderators (firstname,lastname,otchestvo,phone,email,password,nap) 
	...
	if (isset($_POST['login_user'])) {
		...
		if (count($errors) == 0) {
			$query = "SELECT * FROM moderators WHERE email='$email' AND password='$password'";
